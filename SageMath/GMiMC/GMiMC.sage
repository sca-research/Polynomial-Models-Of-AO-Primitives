...
class GMiMC:
    """
    Implementation of GMiMC-erf and GMiMC-crf with multivariate key.
    """
    def __init__(self,
                 field=GF(2**3),
                 n=5,
    ...
    def admissible_key_schedule_matrix(self):
        ...
        for i in range(0, ceil(self.r / self.n) - 1):
    ...
    def key_schedule(self, master_key):
        """
        Computes GMiMC round keys with key schedule matrix.
        
        INPUT:
        "master_key" -- Expects a list or vector with n field elements.
        
        OUTPUT:
        Returns a list of r round keys.
        """
        round_keys = []
        tmp_key = vector(copy(master_key))
        round_keys = round_keys + list(master_key)
        for i in range(0, ceil(self.r / self.n) - 1):
            tmp_key = self.key_schedule_matrix * tmp_key
            round_keys = round_keys + list(tmp_key)
        return round_keys
    ...
    def encrypt(self, plain, key):
        ...
        key = vector(self.field, copy(key))
    ...
    def generate_polynomials(self,
        ...
        key_variables = vector(P, variables[self.n * (self.r - 1):])
        round_keys = self.key_schedule(key_variables)
    ...
    def generic_coordinates(self,
        ...
        variables = P.gens()
        key_variables = variables[self.n * (self.r - 1):]
        round_keys = self.key_schedule(key_variables)
        ...
        lin_polys_top.append(round_keys[0])
        if self.mode == "erf":
            for i in range(1, self.r):
                lin_polys_top.append(variables[self.n * (i - 1)] + round_keys[i])
        elif self.mode == "crf":
            for i in range(1, self.r):
                lin_polys_top.append(sum(variables[self.n * (i - 1) + 1:self.n * i]) + round_keys[i])
    ...
    def compute_Groebner_basis(self,
        ...
        poly = round_keys[0] + self.constants[0] - variables_subs[0]
        substitution_polys.append(poly)
        if self.mode == "erf":
            for i in range(1, self.r):
                poly = variables[self.n * (i - 1)] + round_keys[i]  + self.constants[i] - variables_subs[i]
                substitution_polys.append(poly)
        elif self.mode == "crf":
            for i in range(1, self.r):
                poly = sum(variables[self.n * (i - 1) + 1:self.n * i]) + round_keys[i] + self.constants[i] - variables_subs[i]
                substitution_polys.append(poly)
